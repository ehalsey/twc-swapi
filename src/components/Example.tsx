...
export class VerticalStackConfigureExample extends React.Component<{}, IExampleOptions> {
  ...
  private _onBoxShadowChange = (ev?: React.FormEvent<HTMLElement | HTMLInputElement>, isChecked?: boolean): void => {
    this.setState({ showBoxShadow: isChecked ? isChecked : false });
  };

  private _onPreventOverflowChange = (ev?: React.FormEvent<HTMLElement | HTMLInputElement>, isChecked?: boolean): void => {
    this.setState({ preventOverflow: isChecked ? isChecked : false });
  };

  private _onShrinkItemsChange = (ev?: React.FormEvent<HTMLElement | HTMLInputElement>, isChecked?: boolean): void => {
    this.setState({ disableShrink: !isChecked });
  };

  private _onWrapChange = (ev?: React.FormEvent<HTMLElement | HTMLInputElement>, isChecked?: boolean): void => {
    this.setState({ wrap: isChecked ? isChecked : false });
  };
  ...
  private _onAutoHeightChange = (ev?: React.FormEvent<HTMLElement | HTMLInputElement>, isChecked?: boolean): void => {
    this.setState({ autoHeight: isChecked ? isChecked : false });
  };
  ...
  private _onVerticalAlignChange = (event: React.FormEvent<HTMLDivElement>, option?: IDropdownOption, index?: number): void => {
    if (option) {
      this.setState({ verticalAlignment: option.key as IStackProps['verticalAlign'] });
    }
  };

  private _onHorizontalAlignChange = (event: React.FormEvent<HTMLDivElement>, option?: IDropdownOption, index?: number): void => {
    if (option) {
      this.setState({ horizontalAlignment: option.key as IStackProps['horizontalAlign'] });
    }
  };

  private _onHideEmptyChildrenChange = (ev?: React.FormEvent<HTMLElement | HTMLInputElement>, isChecked?: boolean): void => {
    this.setState({ hideEmptyChildren: isChecked ? isChecked : false });
  };

  private _onEmptyChildrenChange = (event: React.FormEvent<HTMLInputElement | HTMLTextAreaElement>, value?: string): void => {
    if (value === undefined) {
      return;
    }
